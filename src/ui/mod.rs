...
impl DirtyRegion {
    ...
    pub fn invalidate_omnibox(&mut self) {
        self.omnibox = true;
    }
    pub fn invalidate_tabbar(&mut self) {
        self.tabbar = true;
    }
    ...
    pub fn needs_redraw(&self) -> bool {
        self.omnibox || self.tabbar || self.whole_window
    }
...
pub fn hit_test_tab_bar(cursor_x: f64, num_tabs: usize, window_width: f64) -> TabHit {
    ...
    for i in 0..num_tabs {
        ...
        if start_x >= window_width as usize {
            break;
        }
...
fn draw_char_clipped(
    ...
    let min_y = clip.y.min(buf_height).max(y);
    let max_y = clip
        .y
        .saturating_add(clip.h)
        .min(buf_height)
        .min(y.saturating_add(16));
    if min_y >= max_y {
        return;
    }

    let min_x = clip.x.min(buf_width).max(x);
    let max_x = clip
        .x
        .saturating_add(clip.w)
        .min(buf_width)
        .min(x.saturating_add(8));
    if min_x >= max_x {
        return;
    }
    ...
    if min_x == x && max_x == x + 8 && min_y == y && max_y == y + 16 {
        for (row_idx, &row_val) in glyph.iter().enumerate() {
            ...
            if (row_val & 0x80) != 0 {
                buffer[row_base] = color;
            }
            if (row_val & 0x40) != 0 {
                buffer[row_base + 1] = color;
            }
            if (row_val & 0x20) != 0 {
                buffer[row_base + 2] = color;
            }
            if (row_val & 0x10) != 0 {
                buffer[row_base + 3] = color;
            }
            if (row_val & 0x08) != 0 {
                buffer[row_base + 4] = color;
            }
            if (row_val & 0x04) != 0 {
                buffer[row_base + 5] = color;
            }
            if (row_val & 0x02) != 0 {
                buffer[row_base + 6] = color;
            }
            if (row_val & 0x01) != 0 {
                buffer[row_base + 7] = color;
            }
        }
        return;
    }
...
pub fn draw_char(buffer: &mut [u32], buf_width: usize, x: usize, y: usize, c: char, color: u32) {
    let buf_height = buffer.len().checked_div(buf_width).unwrap_or(0);
    draw_char_clipped(
        buffer,
        buf_width,
        x,
...
pub fn render_tab_bar(buffer: &mut [u32], width: usize, tabs: &[Tab], active_index: usize) {
    ...
    for (i, tab) in tabs.iter().enumerate() {
        ...
        let w = if start_x + TAB_WIDTH > width {
            width - start_x
        } else {
            TAB_WIDTH
        };
        ...
        let title_to_draw = if tab.title.is_empty() {
            "Nova Aba"
        } else {
            &tab.title
        };
        let active_fg = if is_active {
            0xFF_FF_FF_FF
        } else {
            0xFF_99_99_99
        };
