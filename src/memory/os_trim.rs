        ...
        if should_trim {
            ...
            if let Err(e) = execute_host_trim() {
                if !self.logged_trim_error {
                    eprintln!(
                        "[Guardião de Memória] Aviso: Trim do Host ignorado. (Motivo: {})",
                        e
                    );
            ...
            if let Some(wv) = webview {
                let _ = execute_webview_trim_windows(wv);
            }
        }
...
fn get_webview_rss(webview: &wry::WebView) -> Result<usize, String> {
    ...
    unsafe {
        ...
        core_8
            ...
            .map_err(|e| format!("BrowserProcessId falha: {}", e))?;
        if pid == 0 {
            return Err("PID zero".into());
        }
        let handle = OpenProcess(PROCESS_QUERY_INFORMATION, 0, pid);
        if handle == 0 {
            return Err("OpenProcess falhou".into());
        }
        ...
        CloseHandle(handle);
        if success == 0 {
            return Err("GetProcessMemoryInfo falhou".into());
        }
...
fn execute_host_trim() -> Result<(), String> {
    ...
    unsafe {
        if EmptyWorkingSet(GetCurrentProcess()) == 0 {
            return Err("EmptyWorkingSet falhou".into());
        }
    }
    Ok(())
}
...
fn execute_webview_trim_windows(webview: &wry::WebView) -> Result<(), String> {
    ...
    unsafe {
        ...
        let core = controller
            .CoreWebView2()
            .map_err(|e| format!("Get CoreWebView2 falhou: {}", e))?;
        let core_8 = core
            .cast::<ICoreWebView2_8>()
            .map_err(|e| format!("ICoreWebView2_8 indisponível: {}", e))?;
        let mut pid: u32 = 0;
        core_8
            .BrowserProcessId(&mut pid)
            .map_err(|e| format!("BrowserProcessId falhou: {}", e))?;
        if pid == 0 {
